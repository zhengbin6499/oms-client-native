...
MSDKVideoDecoder::MSDKVideoDecoder(webrtc::VideoCodecType type,
    ...
    : codecType_(type), inited_(false), decoder_thread_(new rtc::Thread()) {
  ...
  external_d3d11_device_ = external_device;
}
...
mfxStatus MSDKVideoDecoder::CreateD3D11Resources() {
  ...
  sts = d3d11_device_->Init(nullptr,
                            ...
                            external_d3d11_device_);
  if (sts < MFX_ERR_NONE) {
    RTC_LOG(LS_ERROR) << "d3d11_device_->Init() failed with code:" << sts;
    return sts;
  }
  ...
  if (sts < MFX_ERR_NONE || hdl == nullptr) {
    RTC_LOG(LS_ERROR) << "d3d11_device->GetHandle() failed.";
    return MFX_ERR_NULL_PTR;
  }
  sts = mfx_session_->SetHandle(handle_type, hdl);
  if (sts < MFX_ERR_NONE) {
    RTC_LOG(LS_ERROR) << "mfx_session->SetHandle() failed with sts:" << sts;
    return sts;
  }
  ...
  if (sts < MFX_ERR_NONE) {
    RTC_LOG(LS_ERROR) << "mfx_session->SetFrameAllocator() failed with sts:"
                      << sts;
    return sts;
  }
  ...
  if (sts < MFX_ERR_NONE) {
    RTC_LOG(LS_ERROR) << "d3d11_frame_allocator->Init() failed with sts:"
                      << sts;
    return sts;
  }
...
int32_t MSDKVideoDecoder::Decode(
  ...
  ReadFromInputStream(&mfx_bs_, inputImage._buffer, inputImage._length);
...
dec_header:
  if (inited_ && !mfx_video_param_extracted_) {
    ...
    if (MFX_ERR_NONE == sts || MFX_WRN_PARTIAL_ACCELERATION == sts) {
      ...
      sts = d3d11_frame_allocator_->Alloc(d3d11_frame_allocator_->pthis,
                                          &request, &mfx_alloc_response_);
      if (MFX_ERR_NONE != sts) {
