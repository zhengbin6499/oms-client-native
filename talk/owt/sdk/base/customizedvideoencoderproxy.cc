...
int32_t CustomizedVideoEncoderProxy::Encode(
  ...
  if (!update_ts_) {
    ...
    encodedframe.SetTimestamp(last_timestamp_);
  } else {
    last_capture_timestamp_ = encodedframe.capture_time_ms_;
    last_timestamp_ = encodedframe.Timestamp();
  }
  ...
  if (codec_type_ == webrtc::kVideoCodecVP8) {
    ...
    // Phase 1 gaming setting. Will come up with more accurate settings following
    // vp9 depacketizer impl.
    bool vp9_key_frame = (encodedframe._frameType == webrtc::VideoFrameType::kVideoFrameKey);
    if (vp9_key_frame)
      gof_idx_ = 0;
    info.codecSpecific.VP9.inter_pic_predicted = vp9_key_frame ? false : true;
...
void CustomizedVideoEncoderProxy::SetRates(
  ...
  if (encoder_event_callback_) {
    encoder_event_callback_->RequestRateUpdate(parameters.bandwidth_allocation.bps(),
