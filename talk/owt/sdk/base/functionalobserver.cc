      ...
      switch (report_type) {
        ...
        case REPORT_AUDIO_SENDER: {
          std::unique_ptr<AudioSenderReport> audio_send_report_ptr(
              new AudioSenderReport(
                  ...
                  report
                      ->FindValue(webrtc::StatsReport::kStatsValueNameCodecName)
                      ->string_val()));
        ...
        case REPORT_VIDEO_SENDER: {
          ...
          std::unique_ptr<VideoSenderReport> video_send_report_ptr(
              new VideoSenderReport(
                  ...
                  GetValue<int>(
                      &webrtc::StatsReport::Value::int_val, report,
                      webrtc::StatsReport::kStatsValueNameFirsReceived, 0),
                  GetValue<int>(
                      &webrtc::StatsReport::Value::int_val, report,
                      webrtc::StatsReport::kStatsValueNamePlisReceived, 0),
                  GetValue<int>(
                      &webrtc::StatsReport::Value::int_val, report,
                      webrtc::StatsReport::kStatsValueNameNacksReceived, 0),
                  GetValue<int>(
                      &webrtc::StatsReport::Value::int_val, report,
                  ...
                  GetValue<int>(
                      &webrtc::StatsReport::Value::int_val, report,
                      webrtc::StatsReport::kStatsValueNameFrameRateSent, 0),
                  adapt_reason,
                  GetValue<int>(
                      &webrtc::StatsReport::Value::int_val, report,
        ...
        case REPORT_VIDEO_BWE: {
          ...
          connection_stats->video_bandwidth_stats.retransmit_bitrate =
              GetValue<int>(
                  &webrtc::StatsReport::Value::int_val, report,
                  webrtc::StatsReport::kStatsValueNameRetransmitBitrate, 0);
          break;
        }
        default:
          break;
...
IceCandidateType FunctionalStatsObserver::GetCandidateType(
...
TransportProtocolType FunctionalStatsObserver::GetTransportProtocolType(
...
std::shared_ptr<IceCandidateReport>
FunctionalStatsObserver::GetIceCandidateReport(
    const webrtc::StatsReport& report) {
  std::shared_ptr<IceCandidateReport> candidate_report_ptr(
      new IceCandidateReport(
          report.id()->ToString(),
          report
              .FindValue(webrtc::StatsReport::kStatsValueNameCandidateIPAddress)
              ->string_val(),
          ...
          GetTransportProtocolType(
              report
                  ...
                  ->string_val()),
          GetCandidateType(std::string(
              report
                  .FindValue(webrtc::StatsReport::kStatsValueNameCandidateType)
                  ->static_string_val())),
          report
              .FindValue(webrtc::StatsReport::kStatsValueNameCandidatePriority)
              ->int_val()));
...
void FunctionalStandardRTCStatsCollectorCallback::OnStatsDelivered(
  ...
  // owt_stats_delivered_ callback. Currently we filter the report to only below
  // reports: "data-channel", "candidate-pair", "track", "inbound-rtp",
  // "outbound-rtp",
    ...
    if (strcmp(stats.type(), RTCStatsType::kDataChannel) == 0) {
      ...
      std::unique_ptr<owt::base::RTCInboundRTPStreamStats> stat =
          std::make_unique<owt::base::RTCInboundRTPStreamStats>(
              ...
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, packets_received, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, fec_packets_received, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, fec_packets_discarded, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, bytes_received, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, header_bytes_received, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, packets_lost, int32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats,
                                         last_packet_received_timestamp, double, -1),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, jitter, double, -1),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, round_trip_time, double, -1),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, packets_discarded, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, packets_repaired, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, burst_packets_lost, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats,
                                         burst_packets_discarded, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, burst_loss_count, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, burst_discard_count, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, burst_loss_rate, double, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, burst_discard_rate, double, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, gap_loss_rate, double, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, gap_discard_rate, double, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, frames_decoded, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, key_frames_decoded, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, total_decode_time, double, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats,
                                         total_inter_frame_delay, double, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats,
                                         total_squared_inter_frame_delay, double, 0),
              webrtc_stats.content_type.is_defined()
                  ? webrtc_stats.content_type.ValueToString() : "",
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, estimated_playout_timestamp, double, 0),
      ...
      std::unique_ptr<owt::base::RTCOutboundRTPStreamStats> stat =
          std::make_unique<owt::base::RTCOutboundRTPStreamStats>(
              ...
              webrtc_stats.remote_id.is_defined()
                  ...
                  : "",
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, packets_sent, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats,
                                         retransmitted_packets_sent, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, bytes_sent, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, header_bytes_sent, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, retransmitted_bytes_sent, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, target_bitrate, double, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, frames_encoded, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, key_frames_encoded, uint32_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, total_encode_time, double, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats,
                                         total_encoded_bytes_target, uint64_t, 0),
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats, total_packet_send_delay, double, 0),
              webrtc_stats.quality_limitation_reason.is_defined()
                  ? webrtc_stats.quality_limitation_reason.ValueToString()
                  : "",
              OWT_STATS_VALUE_OR_DEFAULT(webrtc_stats,
                                         quality_limitation_resolution_changes, uint32_t, 0),
              webrtc_stats.content_type.is_defined()
                  ? webrtc_stats.content_type.ValueToString()
                  : "",
